...
function ExploreState:init()
    ...
    gSounds['music']:stop()
    gSounds['speedmusic']:setLooping(true)
    gSounds['speedmusic']:setVolume(VOLUME)
    gSounds['speedmusic']:play()
    self.camX = 0
    self.camY = 0
    self.level = LevelMaker.generate(500, 15)
    self.length = 500
...
function ExploreState:update(dt)
    ...
    if math.floor(self.player.x / TILE_SIZE) == self.length - 1 and self.keyIsTaken == true and self.player.hasHit then
        ...
        self.keyY = math.random(VIRTUAL_HEIGHT / 4)
        self.keyIsTaken = false
        PLAYER_WALK_SPEED = math.min(PLAYER_WALK_SPEED * 1.4, 2000)
    end
...
function ExploreState:render()
    ...
    love.graphics.setColor(0, 0, 0, 1)
    love.graphics.print('Level ' .. tostring(LEVEL_NUM) .. 'E', 5, VIRTUAL_HEIGHT - 25)
    love.graphics.setColor(0, 0, 1, 1)
    love.graphics.print('Level ' .. tostring(LEVEL_NUM) .. 'E', 4, VIRTUAL_HEIGHT - 25)
...
function ExploreState:updateCamera()
...
function ExploreState:spawnEnemies()
